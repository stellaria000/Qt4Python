...
class MdiChild(QTextEdit):
    ...
    def save(self):
        if self._is_untitled: return self.save_as()
        else: return self.save_file(self._cur_file)
    ...
    def user_friendly_current_file(self):  return self.stripped_name(self._cur_file)

    def current_file(self): return self._cur_file

    def closeEvent(self, event):
        if self.maybe_save():  event.accept()
        else: event.ignore()

    def document_was_modified(self): self.setWindowModified(self.document().isModified())

    def maybe_save(self):
        if self.document().isModified():
            f = self.user_friendly_current_file()
            message = f"'{f}' has been modified.\nDo you want to save your changes?"
            ret = QMessageBox.warning(self, "MDI", message, QMessageBox.Save | QMessageBox.Discard | QMessageBox.Cancel)
    ...
    def stripped_name(self, fullFileName): return QFileInfo(fullFileName).fileName()
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        if self._mdi_area.currentSubWindow(): event.ignore()
        else:
            self.write_settings()
            event.accept()
    ...
    @Slot()
    def about(self):
        QMessageBox.about(self, "About MDI", "The <b>MDI</b> example demonstrates how to write multiple "
    ...
    def active_mdi_child(self):
        ...
        if active_sub_window: return active_sub_window.widget()
    ...
    def find_mdi_child(self, fileName):
        ...
        for window in self._mdi_area.subWindowList():
            if window.widget().current_file() == canonical_file_path: return window
        
        return None

    @Slot()
    def switch_layout_direction(self):
        if self.layoutDirection() == Qt.LeftToRight: QApplication.setLayoutDirection(Qt.RightToLeft)
        else: QApplication.setLayoutDirection(Qt.LeftToRight)

    def set_active_sub_window(self, window):
        if window: self._mdi_area.setActiveSubWindow(window)
