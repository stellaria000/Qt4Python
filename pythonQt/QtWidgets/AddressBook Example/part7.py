...
from PySide6.QtCore import Qt, Slot, QFile, QIODevice, QTextStream
...
class SortedDict(dict):
    class Iterator(object):
        ...
        def next(self):
            if self._idx>= self._nr_items: raise StopIteration
...
class AddressBook(QWidget):
    ...
    def remove_contact(self):
        ...
        if name in self.contacts:
            button= QMessageBox.question(self, "Confirm Remove", f'Are you sure you want to remove "{name}"?',
                                         QMessageBox.Yes| QMessageBox.No)
            if button== QMessageBox.Yes:
                self.previous()
                del self.contacts[name]

                QMessageBox.information(self, "Remove Successful", f'"{name}" has been removed from your address book.')
    ...
    def save_to_file(self):
        fileName, _= QFileDialog.getSaveFileName(self, "Save, Address Book", '', "Address Book (*.abk);;All Files (*)")

        if not fileName: return

        try: out_file= open(str(fileName), 'wb')
        except IOError:
            QMessageBox.information(self, "Unable to open file", f'There was an error openign "{fileName}"')
            return
    ...
    def load_from_file(self):
        ...
        try: in_file= open(str(fileName), 'rb')
        except IOError:
            QMessageBox.information(self, "Unable to open file", f'There was an error openign "{fileName}"')
            return
        ...
        if len(self.contacts)== 0: QMessageBox.information(self, "No contacts in file",
                                                           "The file you're attempting to open contains no contacts.")
        else:
    ...
    def export_as_vcard(self):
        ...
        file_name= QFileDialog.getSaveFileName(self, "Export Contact", '', "vCard Files (*vcf);; All Files (*)")[0]
        ...
        QMessageBox.information(self, "Export Successful", f'"{name}" has been exported as a vCard.')
