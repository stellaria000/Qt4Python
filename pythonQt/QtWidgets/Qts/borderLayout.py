...
from PySide6.QtWidgets import QLayoutItem, QLayout, QWidget, QWidgetItem, QTextBrowser, QLabel, QFrame, QApplication
...
class BorderLayout(QLayout):
    ...
    def addItem(self, item: QLayoutItem): self.add(item, Position.West)
    def addWidget(self, widget:QWidget, position: Position):
        self.add(QWidgetItem(widget), position)
    def expandingDirections(self)-> Qt.Orientations:
        return Qt.Horizontal| Qt.Vertical

    def hasHeightForWidth(self)-> bool: return False

    def count(self)-> int: return len(self._list)
    ...
    def minimumSize(self)-> QSize: return self.calculate_size(SizeType.MinumumSize)
    ...
    def setGeometry(self, rect: QRect):
        ...
        for wrapper in self._list:
            ...
            if position== Position.North:
                item.setGeometry(QRect(rect.x(), north_height, rect.width(), item.sizeHint().height()))
                ...
                item.setGeometry(QRect(rect.x(), rect.y()+ rect.height()- south_height+ self.spacing(),
                                       item.geometry().width(), item.geometry().height()))
            elif position== Position.Center: center= wrapper
        ...
        for wrapper in self._list:
            ...
            if position== Position.West:
                item.setGeometry(QRect(rect.x()+ west_width, north_height, item.sizeHint().width(), center_height))
                west_width+= item.geometry().width()+ self.spacing()

            elif position == Position.East:
                item.setGeometry(QRect(item.geometry().x(), item.geometry().y(), item.sizeHint().width(), center_height))
                east_width+= item.geometry().width()+ self.spacing()
                item.setGeometry(QRect(item.geometry().x(), item.geometry().y(), item.sizeHint().width(), center_height))

        if center:
            center.item.setGeometry(QRect(west_width, north_height, rect.width()- east_width- west_width, center_height))

    def sizeHint(self)-> QSize: return self.calculate_size(SizeType.SizeHint)
    ...
    def add(self, item: QLayoutItem, position: Position): self._list.append(ItemWrapper(item, position))
    ...
    def calculate_size(self, size_type: SizeType):
        ...
        for wrapper in self._list:
            ...
            if size_type== SizeType.MinumumSize: item_size= wrapper.item.minimumSize()
            else: item_size= wrapper.item.sizeHint()
            ...
            if position in (Position.West, Position.East, Position.Center):
                total_size.setHeight(total_size.width()+ item_size.width())
