...
THE TAB DIALOG EXAMPLE CONSISTS OF A SINGLE TABDIALOG CLASS THAT
PROVIDES THREE TABS, EACH CONTAINING INFORMATION ABOUT A PARTICULAR FILE,
AND TWO STANDARD PUSH BUTTONS THAT ARE SUED TO ACCEPT
OR REJECT THE CONTENTS OF THE DIALOG.
'''
...
class PermissionsTab(QWidget):
    def __init__(self, file_info: QFileInfo, parent:QWidget):
        ...
        if file_info.isReadable(): readable.setChecked(True)
        writable= QCheckBox("Writable")
        if file_info.isWritable(): writable.setChecked(True)
        executable= QCheckBox("Executable")
        if file_info.isExecutable(): executable.setChecked(True)
        ...
        group_value_label= QLabel(file_info.owner())
...
class ApplicationsTab(QWidget):
    def __init__(self, file_info: QFileInfo, parent: QWidget):
        ...
        if not file_info.suffix():
            always_check_box= QCheckBox(f"Always use this application to open this type of file")
        else:
