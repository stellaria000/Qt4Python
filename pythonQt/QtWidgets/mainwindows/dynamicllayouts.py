'''
    DYNAMIC LAYOUTS IMPLEMENTS DYNAMICALLY PLACED WIDGETS WITHIN RUNNING APPLICATIONS.
    THE WIDGET PLACEMENT DEPENDS ON WHETHER HORIZONTAL OR V ERTICAL IS CHOSEN.
'''
from PyQt6.QtWidgets import QLayout
from PySide6.QtCore import Qt, QSize
from PySide6.QtWidgets import QDialog, QGridLayout, QMessageBox, QGroupBox, QSpinBox, QSlider, QDial, QProgressBar, \
    QLabel, QComboBox, QDialogButtonBox, QApplication
...
class Dialog(QDialog):
    ...
    def rotate_widgets(self):
        ...
        if count% 2== 1: raise AssertionError("Number of widgets must be even")

        for widget in self._rotable_widgets: self._rotable_widgets.remove(widget)

        self._rotable_widgets.append(self._rotable_widgets.pop(0))

        for i in range(count// 2):
            self._rotable_layout.addWidget(self._roatable_widgets[count- i- 1], 0, i)
            self.rotable_layout.addWidget(self._roatable_widgets[i], 1, i)
    ...
    def buttons_orientation_changed(self, index):
        ...
        if orientation== self._button_box.orientation(): return
    ...
    def create_rotable_group_box(self):
        self._rotable_group_box= QGroupBox("Roatble Widgets")
