...
class DropArea(QLabel):
    ...
    def dropEvent(self, event):
        ...
        if mime_data.hasImage(): self.setPixmap(QPixmap(mime_data.imageData()))
            ...
            for i in range(0, min(len(url_list), 32)): text+= url_list[i].path()+ "\n"

            self.setText(text)
        else: self.setText("Cannot display data")
