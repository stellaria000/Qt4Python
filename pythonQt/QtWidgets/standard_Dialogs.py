...
class DialogOptionsWidget(QGroupBox):
    ...
    def value(self):
        ...
        for checkbox, value in self._mapping.items():
            if checkbox.isChecked(): result|= value
        return result
...
class Dialog(QDialog):
    def __init__(self, parent=None):
        ...
        self._file_options.add_checkbox("Do not use native dialog", QFileDialog.DontUseNativeDialog)
        self._file_options.add_checkbox("Show directories only", QFileDialog.ShowDirsOnly)
        self._file_options.add_checkbox("Do not resolve symlinks", QFileDialog.DontResolveSymlinks)
        self._file_options.add_checkbox("Do not confirm overwrite", QFileDialog.DontConfirmOverwrite)
        self._file_options.add_checkbox("Readonly", QFileDialog.ReadOnly)
        self._file_options.add_checkbox("Hide name filter details", QFileDialog.HideNameFilterDetails)
        self._file_options.add_checkbox("Do not use custom directory icons (Windows)", QFileDialog.DontUseCustomDirectoryIcons)
    ...
    def set_integer(self):
        i, ok = QInputDialog.getInt(self,"QInputDialog.getInteger()", "Percentage:",
                                    25, 0, 100, 1)
        if ok: self._integer_label.setText(f"{i}%")

    @Slot()
    def set_double(self):
        d, ok = QInputDialog.getDouble(self, "QInputDialog.getDouble()","Amount:",
                                       37.56, -10000, 10000, 2)
        if ok: self._double_label.setText(f"${d:g}")
    ...
    def set_font(self):
        ...
        default_font = QFont()
        if description: default_font.fromString(description)
    ...
    def set_existing_directory(self):
        ...
        if directory: self._directory_label.setText(directory)
    ...
    def set_open_file_name(self):
        ...
        if fileName: self._open_file_name_label.setText(fileName)
    ...
    def set_open_file_names(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(self, "QFileDialog.getOpenFileNames()",
                                                self._open_files_path, "All Files (*);;Text Files (*.txt)", "", options)
        if files:
    ...
    def set_save_file_name(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self, "QFileDialog.getSaveFileName()",
                                                  ...
                                                  "All Files (*);;Text Files (*.txt)", "", options)
        if fileName: self._save_file_name_label.setText(fileName)
    ...
    def critical_message(self):
        ...
        msg_box = QMessageBox(QMessageBox.Critical, "QMessageBox.critical()",
                              ...
                              QMessageBox.Abort | QMessageBox.Retry | QMessageBox.Ignore, self)
        msg_box.setInformativeText(m)
        reply = msg_box.exec()
        if reply == QMessageBox.Abort: self._critical_label.setText("Abort")
        elif reply == QMessageBox.Retry: self._critical_label.setText("Retry")
        else: self._critical_label.setText("Ignore")
    ...
    def information_message(self):
        ...
        msg_box = QMessageBox(QMessageBox.Information, "QMessageBox.information()",
                              "Elvis has left the building.", QMessageBox.Ok, self)
        msg_box.setInformativeText(m)
        reply = msg_box.exec()
        if reply == QMessageBox.Ok: self._information_label.setText("OK")
        else: self._information_label.setText("Escape")
    ...
    def question_message(self):
        ...
        msg_box.setInformativeText(m)
        reply = msg_box.exec()
        if reply == QMessageBox.Yes: self._question_label.setText("Yes")
        elif reply == QMessageBox.No: self._question_label.setText("No")
        else: self._question_label.setText("Cancel")
    ...
    def warning_message(self):
        ...
        if msg_box.exec() == QMessageBox.AcceptRole: self._warning_label.setText("Keep")
        else: self._warning_label.setText("Delete")
